...
def initialize_game():
    '''Sets up initial game by dealing and sorting player and AI hands'''
    global AI_hand
    global players_hand
...
def write_AI_hand_to_file(AI_hand):
    '''saves the AI's hand of cards as a menu style view in a text file'''
    fileobject.write("AI Hand " + "(" + str(datetime.datetime.now().strftime("%A, %d. %B %Y %I:%M%p")) + ")" + "\n")
    for index, card in enumerate(AI_hand):
         fileobject.write("{index}) {rank} of {suit} (value: {value})\n".format(index=index+1, rank=card.rank, suit=card.suit, value=card.value))
    fileobject.write("\n")
...
def choose_player_card():
    while True:
        card_index = int(input("Choose a card to play \n"))
        try:
